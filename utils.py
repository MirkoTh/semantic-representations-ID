...
class BatchGenerator_ID(object):
    ...
    def get_batches(self, average_reps:torch.Tensor, triplets:torch.Tensor) -> Iterator[torch.Tensor]:
        ...
        for i in range(self.n_batches):
            #batch = encode_as_onehot(I, triplets[i*self.batch_size: (i+1)*self.batch_size])
            batch = average_reps[triplets.flatten()[i*3*self.batch_size: (i+1)*3*self.batch_size], :]
            yield batch
...
def load_data_ID(device:torch.device, triplets_dir:str, inference:bool=False) -> Tuple[torch.Tensor]:
    ...
    try:
        ...
        print('\n...Could not find any .npy files for current modality.')
        print('...Now searching for .txt files.\n')
        train_triplets = torch.from_numpy(np.loadtxt(pjoin(triplets_dir, 'train_90_ID.txt'))).to(device).type(torch.LongTensor)
        test_triplets = torch.from_numpy(np.loadtxt(pjoin(triplets_dir, 'test_10_ID.txt'))).to(device).type(torch.LongTensor)
    return train_triplets, test_triplets
...
def load_batches_ID(
    ...
    if inference:
        assert train_triplets is None
        test_batches = BatchGenerator_ID(average_reps = average_reps, dataset=test_triplets, batch_size=batch_size, sampling_method=None, p=None)
        return test_batches
    if (multi_proc and n_gpus > 1):
        if sampling_method == 'soft':
            warnings.warn(f'...Soft sampling cannot be used in a multi-process distributed training setting.', RuntimeWarning)
...
def train_ID_model(
    ...
    for epoch in range(start, epochs):
        ...
        for i, batch in enumerate(train_batches_ID):
            ...
            loss = trinomial_loss(anchor, positive, negative, task, temperature, distance_metric)
            ...
            batch_accs_train[i] += choice_accuracy(anchor, positive, negative, task, distance_metric)
            iter += 1
