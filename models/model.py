...
class SPoSE_ID(nn.Module):
    ...
    def _initialize_weights(self) -> None:
        mean_avg, std_avg = 0.1, 0.01
        mean_id, std_id = 0.5, 0.15
        for m in self.modules():
            if isinstance(m, nn.Linear):
...
class SPoSE_ID_Random(nn.Module):
    ...
    def hierarchical_loss(self, id: torch.Tensor):
        ...
        return torch.mean(
            (self.individual_slopes(id) - self.global_mean) ** 2
            / (2 * self.global_std**2)
        )
...
class Scaling_ID_Random(nn.Module):
    ...
    def hierarchical_loss(self, id: torch.Tensor):
        """Encourage slopes to stay within a normal distribution."""
        return torch.mean(
            (torch.exp(self.individual_temps(id)) - self.global_mean) ** 2
            / (2 * self.global_std**2)
        )
...
class CombinedModel(nn.Module):
    def __init__(
        ...
        if scaling == "free":
            ...
            # randomly-varying by-participant softmax temperatures
