...
def run(
    ...
    logger = setup_logging(file='repID_optimization.log', dir=f'./log_files/lmbda_{lmbda}/')
    ...
    if results_dir == './results/':
        results_dir_avg = os.path.join(results_dir, "avg", f'{embed_dim}d', str(lmbda), f'seed{rnd_seed:02d}')
        results_dir_ID = os.path.join(results_dir, "ID", f'{embed_dim}d', str(lmbda), f'seed{rnd_seed:02d}')
    if not os.path.exists(results_dir_ID):
        os.makedirs(results_dir_avg)
        os.makedirs(results_dir_ID)

    if plots_dir == './plots/':
        plots_dir_avg = os.path.join(plots_dir, "avg", f'{embed_dim}d', str(lmbda), f'seed{rnd_seed}')
        plots_dir_ID = os.path.join(plots_dir, "ID", f'{embed_dim}d', str(lmbda), f'seed{rnd_seed}')
    if not os.path.exists(plots_dir_ID):
        os.makedirs(plots_dir_avg)
        os.makedirs(plots_dir_ID)
    ...
    for epoch in tqdm(range(start, epochs)):
        ...
        avg_val_loss, avg_val_acc = ut.validation(model=model, val_batches=val_batches, task=task, device=device, distance_metric=distance_metric)
    ...
    pl.plot_nneg_dims_over_time(plots_dir=plots_dir_avg, nneg_d_over_time=nneg_d_over_time)
    ...
    pl.plot_single_performance(plots_dir=plots_dir_avg, val_accs=val_accs, train_accs=train_accs)
    ...
    pl.plot_complexities_and_loglikelihoods(plots_dir=plots_dir_avg, loglikelihoods=loglikelihoods, complexity_losses=complexity_losses)
    ...
    # load triplet data only from participants with substantial number of choices
    logger.info(f'\nLoading data for individual differences models')
    ...
    # run linear regression models for each participant
    ...
    l_ID = {"l_train_ID": l_train_ID, "l_val_ID": l_val_ID, "l_val_avg": l_val_avg}
    with open(os.path.join(results_dir_ID, 'results-ID.pkl'), "wb") as f:
        pickle.dump(l_ID, f)
    # process ID results
    logger.info(f'\nprocess ID model results')
    dict_ID_results = ut.process_ID_results(l_train_ID, l_val_ID, l_val_avg, len(l_train_triplets_ID), embed_dim)
    logger.info(f'\nsave plots of ID model results')
    pl.plot_ID_training_improvement(dict_ID_results["df_train_acc_agg"], plots_dir_ID)
    pl.plot_ID_vs_avg_eval(dict_ID_results["df_swarm"], len(l_train_triplets_ID), plots_dir_ID)
    pl.plot_parameter_correlations(dict_ID_results["df_params"], plots_dir_ID)
