...
def plot_ID_training_improvement(df_train_acc_agg: pd.DataFrame, plots_dir_ID: str) -> None:
    ...
    ax.grid(True)
    plt.savefig(
        os.path.join(plots_dir_ID, 'training-accuracy-over-epochs.png'),
          dpi=150, bbox_inches='tight'
    )
    plt.close()

def plot_ID_vs_avg_eval(df_swarm: pd.DataFrame, n_ID: int, plots_dir_ID: str) -> None:
    means = df_swarm.groupby('variable')['value'].mean()
    stds = df_swarm.groupby('variable')['value'].std()
    stds = stds /np.sqrt(n_ID)
    # Plot the line without crossing the points 
    ...
    ax.legend(loc=10)
    plt.savefig(
        os.path.join(plots_dir_ID, 'ID-vs-avg.png'),
          dpi=150, bbox_inches='tight'
    )
    plt.close()

def plot_parameter_correlations(df_params: pd.DataFrame, plots_dir_ID: str) -> None:
    # Plot the line without crossing the points 
    ...
    ax.grid(True)
    plt.savefig(
        os.path.join(plots_dir_ID, 'parameter-correlations.png'),
