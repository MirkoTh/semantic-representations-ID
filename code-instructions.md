...
- In the following, the root folder of the project is represented by "~/"
...
From https://osf.io/z2784/files/osfstorage, download the following files: labels.mat, words.mat, unique_id.txt, from https://osf.io/f5rn6/overview download the file called triplets_large_final_correctednc_correctedorder.csv.
...
After you have run the models, you can analyze the results with a set of jupyter notebooks.
...
## Study 2
First, we have to create the triplet set used in Study 2. Then, we can run the study with jatos.
Please download the images from the source study, so that they can be displayed during the study. They are available as a zip file here: osf.io/jum2f/overview. Place the unzipped file called "images" into the ~/data/ folder.
...
When using the model reported in the manuscript with freely varying dimensional weights (i.e., not modeled as random effects), the overlap with the original triplet set, however, is substantial. For the model-based part of the triplet set, the overlap is 166 out of 220 triplets, 171 out of 220 triplets, and 163 out of 220 triplets for dimensionalities 25, 30, and 35, respectively.


### Model deltas
Run the jupyter notebook ~/analyze-model-deltas.ipynb to calculate the differences in prediction accuracy between the average model and the individual differences model. This notebook saves the file "~/data/triplets-delta.csv".
...
### Run the study
We provide a separate folder with all the code and files necessary to run the study in jatos. For details on jatos, visit www.jatos.org. In case you want to re-create the study from scratch using the results from the analyses, you would have to move all the relevant files from the source folder ~/ to the respective jatos folder on your computer.

### Load the data
Run the following three R scripts sequentially: ~/load-study-results.R, ~/exclusion-criteria.R, ~/concatenate-ooo-old-new.R
...
1. R script: plot-figures-ms.R
2. jupyter notebook: plot-figures-ms.ipynb

Note that the overview figures and the first result figure have been manually put together from the resulting figures.
