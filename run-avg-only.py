...
def run(
    ...
    if results_dir == './results/':
        results_dir = os.path.join(
            results_dir, "avg", f'{embed_dim}d', str(lmbda), f'seed{rnd_seed:02d}')
    if not os.path.exists(results_dir):
        os.makedirs(results_dir)
    if plots_dir == './plots/':
        plots_dir = os.path.join(
            plots_dir, "avg", f'{embed_dim}d', str(lmbda), f'seed{rnd_seed}')
    ...
    for epoch in tqdm(range(start, epochs)):
        ...
        for i, batch in enumerate(train_batches):
            ...
            l1_pen = md.l1_regularization(model, "fc.weight").to(device)
            W = model.fc.weight
